...
export default function ChannelAdminsPage() {
  ...
  const sorted = useMemo(() => {
    ...
    const order = (r: string) =>
      r === 'OWNER' ? 0 : r === 'ADMIN' ? 1 : r === 'SUBSCRIBER' ? 2 : 3;
    return [...list].sort((a, b) => order(a.role) - order(b.role));
  }, [chat?.members]);

  const setRole = useMutation({
    mutationFn: (p: { userId: string; role: 'ADMIN' | 'SUBSCRIBER' }) =>
  ...
  return (
    <TgPage className="max-w-lg">
      <TgNav
        title={t('channelSettingsAdminsTitle')}
        backHref={`/chats/${chatId}/channel`}
        backLabel={t('channelSettingsBackToChannelSettings')}
      />
      ...
      <TgCard title={t('channelSettingsAdminsTitle')} className="mt-4">
        {sorted
                ...
                {canDemote ? (
                  <button
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      setRole.mutate({ userId: uid, role: 'SUBSCRIBER' });
                    }}
                  >
      ...
      <TgCard title={t('channelSettingsMembersTitle')} className="mt-6">
        {sorted
          .filter((m) => m.role !== 'OWNER' && m.role !== 'ADMIN')
          .map((m) => {
            const uid = m.user.id ?? m.userId;
              ...
              <TgRow key={uid}>
                ...
                {canPromote ? (
                  <button
                    type="button"
                    disabled={setRole.isPending}
                    className="ml-2 shrink-0 rounded-lg border border-accent/30 bg-accent/10 px-2.5 py-1 text-[13px] font-semibold text-accent disabled:opacity-50"
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
